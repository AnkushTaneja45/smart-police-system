...
router.post('/login', async (req, res) => {
    try {
        ...
        // In prod: const isMatch = await bcrypt.compare(password, user.PasswordHash);
        let isMatch = false;
        // If they enter 'password' or the seed hash, let them in for MVP
        if (password === 'password' || user.PasswordHash === password || user.passwordhash === password) {
            isMatch = true;
        }
